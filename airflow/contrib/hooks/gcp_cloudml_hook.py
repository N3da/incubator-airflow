...
class CloudMLHook(GoogleCloudBaseHook):
    ...
    def create_version(self, project_name, model_name, version_spec):
        ...
        Retrusn the operation if version was created successfully and raises an error otherwise.
        """
        ...
        get_request = self._cloudml.projects().operations().get(name=response['name'])

        return _retry_with_exponential_delay(request = get_request,
                                             max_n = 9,
    ...
    def list_versions(self, project_name, model_name):
        ...
        """
        result = []
        full_parent_name = 'projects/{}/models/{}'.format(project_name, model_name)
        ...
        while next_page_token is not None:
            next_request = self._cloudml.projects().models().versions().list(
                parent=full_parent_name, pageToken=next_page_token, pageSize=100)
